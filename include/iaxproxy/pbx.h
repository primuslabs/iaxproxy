 ...
 * iaxproxy switching architecture.
 *
 * \retval 0 success
 ...
 * 
 * Unregisters a switch from iaxproxy.
 *
 * \return nothing
 */
...
/*! \brief The result codes when starting the PBX on a channel with \see ast_pbx_start.
	AST_PBX_CALL_LIMIT refers to the maxcalls call limit in iaxproxy.conf
 */
enum ast_pbx_result {
	AST_PBX_SUCCESS = 0,
	AST_PBX_FAILED = -1,
 ...
 *
 * This adds a new extension to the iaxproxy extension list.
 *
 * \note It is possible for autoservice to be started and stopped on c during this
 * function call, it is important that c is not locked prior to calling this. Otherwise
 * a deadlock may occur
 ...
 *
 * This function registers a switch with the iaxproxy switch architecture
 *
 * \retval 0 on success 
 * \retval -1 on failure
 */
