 ...
 *
 * This simple C application allows you to control iaxproxy thru one TCP/IP
 ...
 * 1) Main thread - reads commands from the socket and sends them to iaxproxy
 * 2) stdin_thread - reads iaxproxy output and sends it to the command socket
 * 3) signal_thread - reads the sound from iaxproxy and sends it to the signal socket
 */
