 ...
 * for operations outside of iaxproxy. A huge, awful hack.
 *
 */
#include "iaxproxy.h"
...
#include "iaxproxy/compat.h"
#include "iaxproxy/paths.h"	/* we use AST_CONFIG_DIR */
...
#include "iaxproxy/inline_api.h"
#include "iaxproxy/endian.h"
...
#include "iaxproxy/ael_structs.h"
#include "iaxproxy/pval.h"
...
enum ast_option_flags {
	...
	/*! Execute an iaxproxy CLI command upon startup */
...
static struct ast_config *ast_config_internal_load(const char *filename, struct ast_config *cfg, int withcomments, const char *suggested_incl_file)
{
	...
	*/
	...
	if (strcmp(filename, extconfig_conf) && strcmp(filename, "iaxproxy.conf") && config_engine_list) {
	...
	if (cur[0] == '[') {
		...
		*c++ = '\0';
		...
		if (do_exec && !ast_opt_exec_includes) {
			ast_log(LOG_WARNING, "Cannot perform #exec unless execincludes option is enabled in iaxproxy.conf (options section)!\n");
			do_exec = 0;
		}
