...
static char *config = "/etc/iaxproxy/muted.conf";
...
/*! Connect to the iaxproxy manager interface */
static int connect_iaxproxy(void)
{
	int sock;
	struct hostent *hp;
	char *ports;
...
/*! Login to the iaxproxy manager interface */
static int login_iaxproxy(void)
{
	char *welcome;
	char *resp;
	if (!(welcome = get_line())) {
...
int main(int argc, char *argv[])
{
	...
	if (needfork) {
		...
		const char *found = NULL, *paths[] = {
			"/Library/LaunchAgents/org.iaxproxy.muted.plist",
			"/Library/LaunchDaemons/org.iaxproxy.muted.plist",
			"contrib/init.d/org.iaxproxy.muted.plist",
			"<path-to-iaxproxy-source>/contrib/init.d/org.asterisk.muted.plist" };
		char userpath[256];
		struct stat unused;
