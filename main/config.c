...
#include "iaxproxy/paths.h"	/* use ast_config_AST_CONFIG_DIR */
#include "iaxproxy/network.h"	/* we do some sockaddr manipulation here */
#include <time.h>
#include <sys/stat.h>
...
#include "iaxproxy/strings.h"	/* for the ast_str_*() API */
...
int read_config_maps(void) 
{
	...
	for (v = ast_variable_browse(config, "settings"); v; v = v->next) {
		...
		if (!strcmp(v->name, "iaxproxy.conf")) {
			ast_log(LOG_WARNING, "Cannot bind 'iaxproxy.conf'!\n");
			continue;
		}
