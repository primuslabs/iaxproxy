...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 299130 $")
...
static int config_module(int reload)
{
	...
	if (!(tmp = ast_variable_retrieve(cfg, "global", "dbname"))) {
		ast_log(LOG_WARNING, "PostgreSQL database not specified.  Assuming iaxproxy\n");
		tmp = "iaxproxycdrdb";
	}
	...
	if (!(tmp = ast_variable_retrieve(cfg, "global", "user"))) {
		ast_log(LOG_WARNING, "PostgreSQL database user not specified.  Assuming iaxproxy\n");
		tmp = "iaxproxy";
	}
