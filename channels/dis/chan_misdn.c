...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 284593 $")
...
#include "iaxproxy/frame.h"
#include "iaxproxy/translate.h"
#include "iaxproxy/cli.h"
#include "iaxproxy/musiconhold.h"
#include "iaxproxy/dsp.h"
#include "iaxproxy/file.h"
#include "iaxproxy/callerid.h"
#include "iaxproxy/indications.h"
#include "iaxproxy/app.h"
#include "iaxproxy/features.h"
#include "iaxproxy/term.h"
#include "iaxproxy/sched.h"
#include "iaxproxy/stringfields.h"
#include "iaxproxy/abstract_jb.h"
#include "iaxproxy/causes.h"
...
enum misdn_chan_state {
	...
	MISDN_EXTCANTMATCH,        /*!< when iaxproxy couldn't match our ext */
...
struct chan_list {
	/*! 
	 * \brief The "allowed_bearers" string read in from /etc/iaxproxy/misdn.conf
	 */
	...
	/*!
	 * \brief TRUE if a channel can be hung up by calling iaxproxy directly when done.
	 */
	int need_hangup;
	 ...
	 * \note The "noautorespond_on_setup" boolean read in from /etc/iaxproxy/misdn.conf
	 */
	 ...
	 * \note The "incoming_early_audio" boolean read in from /etc/iaxproxy/misdn.conf
	 */
	 ...
	 * \note The "faxdetect" option string read in from /etc/iaxproxy/misdn.conf
	 * \note It is settable by the misdn_set_opt() application.
	 */
	int faxdetect;
	 ...
	 * \note The "faxdetect_timeout" value read in from /etc/iaxproxy/misdn.conf
	 */
	 ...
	 * \note The "astdtmf" boolean read in from /etc/iaxproxy/misdn.conf
	 */
	int ast_dsp;

	/*!
	 * \brief Jitterbuffer length
	 * \note The "jitterbuffer" value read in from /etc/iaxproxy/misdn.conf
	 */
	int jb_len;

	/*!
	 * \brief Jitterbuffer upper threshold
	 * \note The "jitterbuffer_upper_threshold" value read in from /etc/iaxproxy/misdn.conf
	 */
	 ...
	 * \note The "context" string read in from /etc/iaxproxy/misdn.conf
	 */
	char context[AST_MAX_CONTEXT];

	/*!
	 * \brief The configured music-on-hold class to use for this call.
	 * \note The "musicclass" string read in from /etc/iaxproxy/misdn.conf
	 */
	 ...
	 * \note The "far_alerting" boolean read in from /etc/iaxproxy/misdn.conf
	 */
	int far_alerting;

	/*!
	 * \brief TRUE if NT should disconnect an overlap dialing call when a timeout occurs.
	 * \note The "nttimeout" boolean read in from /etc/iaxproxy/misdn.conf
	 */
	int nttimeout;
	 ...
	 * \note The "overlapdial" value read in from /etc/iaxproxy/misdn.conf
	 */
	int overlap_dial;
...
static struct state_struct state_array[] = {
	...
	{ MISDN_EXTCANTMATCH,        "EXTCANTMATCH" },        /*  when iaxproxy couldn't match our ext */
	...
	case EVENT_BCHAN_DATA:
		...
		if (!misdn_cap_is_speech(ch->bc->capability)) {
			...
			if (pfd.revents & POLLOUT) {
				chan_misdn_log(9, bc->port, "writing %d bytes to iaxproxy\n", bc->bframe_len);
