...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 314620 $")
...
static int skinny_indicate(struct ast_channel *ast, int ind, const void *data, size_t datalen)
{
	...
	case AST_CONTROL_RINGING:
		...
		return -1; /* Tell iaxproxy to provide inband signalling */
	case AST_CONTROL_BUSY:
		if (ast->_state != AST_STATE_UP) {
			if (!d->earlyrtp) {
...
static int handle_offhook_message(struct skinny_req *req, struct skinnysession *s)
{
	...
	/* if any line on a device is offhook, than the device must be offhook, 
	   ...
	   but iaxproxy does, so we may need to do more work.  Ugly, we should 
