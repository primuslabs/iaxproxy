...
#include "iaxproxy/jabber.h"
#include "iaxproxy/jingle.h"
...
static int jingle_create_candidates(struct jingle *client, struct jingle_pvt *p, char *sid, char *from)
{
	...
	for (tmp = p->ourcandidates; tmp; tmp = tmp->next) {
		...
		iks_insert_attrib(content, "name", "iaxproxy-audio-content");
...
static int jingle_transmit_invite(struct jingle_pvt *p)
{
	...
	ast_copy_string(p->audio_content_name, "iaxproxy-audio-content", sizeof(p->audio_content_name));
