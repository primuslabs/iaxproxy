 ...
 *	}
 *
 * XXX NOTE these macros should NOT be replicated in other parts of iaxproxy. 
...
static int soundcard_writeframe(struct chan_usbradio_pvt *o, short *data)
{
	...
	//  filling the buffer when iaxproxy clock > usb sound clock
...
static void tune_write(struct chan_usbradio_pvt *o)
{
	...
	char fname[200];
 	...
 	snprintf(fname,sizeof(fname) - 1,"/etc/iaxproxy/usbradio_tune_%s.conf",o->name);
	fp = fopen(fname,"w");
