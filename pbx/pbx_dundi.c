...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 271763 $")
	...
	while(v) {
		if (!strcasecmp(v->name, "port")){
			...
			if(last_port==0){
				...
				ast_log(LOG_WARNING, "change to port ignored until next iaxproxy re-start\n");
