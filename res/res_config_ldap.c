...
#define RES_CONFIG_LDAP_DEFAULT_BASEDN "iaxproxy"
...
* this is an area of iaxproxy that could do with a lot of modification
...
int parse_config(void)
{
	...
	if (!ast_strlen_zero(user)) {
		if (!(s = ast_variable_retrieve(config, "_general", "pass"))) {
			ast_log(LOG_WARNING, "No directory password found, using 'iaxproxy' as default.\n");
			ast_copy_string(pass, "iaxproxy", sizeof(pass));
		} else {
