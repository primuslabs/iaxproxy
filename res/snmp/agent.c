...
/*
 * There is some collision collision between netsmp and iaxproxy names,
...
#include "iaxproxy/paths.h"	/* need ast_config_AST_SOCKET */
...
/* Forward declaration */
static void init_iaxproxy_mib(void);

/*
 * Anchor for all the Asterisk MIB values
 */
static oid iaxproxy_oid[] = { 1, 3, 6, 1, 4, 1, 22736, 1 };
...
static int term_iaxproxy_mib(int majorID, int minorID, void *serverarg, void *clientarg)
{
	unregister_sysORTable(iaxproxy_oid, OID_LENGTH(asterisk_oid));
	return 0;
}

static void init_iaxproxy_mib(void)
{
	static struct variable4 iaxproxy_vars[] = {
	...
	register_sysORTable(iaxproxy_oid, OID_LENGTH(asterisk_oid),
			"ASTERISK-MIB implementation for Asterisk.");

	REGISTER_MIB("res_snmp", iaxproxy_vars, variable4, asterisk_oid);

	snmp_register_callback(SNMP_CALLBACK_LIBRARY,
			   SNMP_CALLBACK_SHUTDOWN,
			   term_iaxproxy_mib, NULL);
}
