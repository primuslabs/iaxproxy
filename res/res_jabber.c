...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 295440 $")
...
static int aji_client_info_handler(void *data, ikspak *pak)
{
	...
	if (pak->subtype == IKS_TYPE_RESULT) {
		...
		if (iq && ident && disco && google) {
			...
			iks_insert_attrib(ident, "name", "iaxproxy");
...
static void aji_set_presence(struct aji_client *client, char *to, char *from, int level, char *desc)
{
	...
	if (presence && cnode && client && priority) {
		...
		iks_insert_node(presence, priority);
		iks_insert_attrib(cnode, "node", "http://www.iaxproxy.org/xmpp/client/caps");
		iks_insert_attrib(cnode, "ver", "iaxproxy-xmpp");
...
static int aji_create_client(char *label, struct ast_variable *var, int debug)
{
	...
	*/
	...
	if (!strchr(client->user, '/') && !client->component) { /*client */
		resource = NULL;
		if (asprintf(&resource, "%s/iaxproxy", client->user) >= 0) {
