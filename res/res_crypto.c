...
#include "iaxproxy/paths.h"	/* use ast_config_AST_KEY_DIR */
	...
	if (key->rsa) {
		...
		if (!notice) {
			if (!ast_opt_init_keys) 
				ast_log(LOG_NOTICE, "Add the '-i' flag to the iaxproxy command line if you want to automatically initialize passcodes at launch.\n");
			notice++;
		}
		/* Keep it anyway */
		key->delme = 0;
